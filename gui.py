...
def extraer_tiempo(texto):
    """
    Busca valores como:
    'Tiempo promedio (QuickSort secuencial): 0.12345 segundos'
    """
...
def ejecutar_programa(comando, variables_entorno=None):
    ...
    try:
        ...
        salida_text.insert(tk.END, f"[ERROR] No fue posible ejecutar el programa.\n{ex}\n")
        return ""
...
def generar_dataset():
    ...
    try:
        ...
        salida_text.insert(tk.END, f"✔ Dataset generado correctamente ({cantidad} registros).\n")

    except Exception as ex:
        salida_text.insert(tk.END, f"❌ Error durante la generación del dataset.\n{ex}\n")
...
btn_dataset = tk.Button(panel_config, text="Generar Dataset", font=("Arial", 11),
                        width=18, command=generar_dataset, bg="#DDEBF7")
...
btn_secuencial = tk.Button(panel_botones, text="Ejecutar Secuencial", width=22,
                           font=("Arial", 12, "bold"), bg="#CFE2F3",
                           command=ejecutar_secuencial)
btn_secuencial.grid(row=0, column=0, padx=10)

btn_openmp = tk.Button(panel_botones, text="Ejecutar OpenMP", width=22,
                       font=("Arial", 12, "bold"), bg="#D9EAD3",
                       command=ejecutar_openmp)
btn_openmp.grid(row=0, column=1, padx=10)

btn_mpi = tk.Button(panel_botones, text="Ejecutar MPI", width=22,
                    font=("Arial", 12, "bold"), bg="#F4CCCC",
                    command=ejecutar_mpi)
btn_mpi.grid(row=0, column=2, padx=10)

# -------- BOTÓN PARA GRAFICAR --------
btn_grafico = tk.Button(ventana, text="📊 Generar Gráfico Comparativo",
                        font=("Arial", 14, "bold"),
                        bg="#FFF2CC", width=30,
...
# -------- SALIDA --------
salida_text = scrolledtext.ScrolledText(ventana, width=115, height=25, font=("Consolas", 10))
